...
export default function Sidebar({ show, setter }: SidebarProps) {
  ...
  return (
    <div className="flex">
      <nav className="invisible md:visible z-20 fixed top-0 left-0 right-0 h-[var(--header-height)] bg-white flex [&>*]:my-auto px-6 border-b-2">
        <Link href="/">
      ...
      <div className={`${className}${appendClass}`}>
        ...
        <div className="flex flex-col border-t-2">
          ...
          <MenuItem name="Timetable" route="/" />
          <MenuItem name="Announcement" route="/" />
        </div>
      </div>
      {show ? <ModalOverlay /> : <></>}
    </div>
